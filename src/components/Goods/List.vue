...
<el-table :data='goodList' border stripe>
  ...
  <el-table-column label="创业时间" prop='add_time' width='140px'>
...
export default {
  ...
  methods: {
    async getGoodList () {
      ...
      if (res.meta.status !== 200) return this.$message.info('获取信息失败')
    ...
    async removeById (gid) {
      ...
      const { data: res1 } = await this.$http.delete(`goods/${gid}`)
